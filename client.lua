    ...
    while true do
        ...
        if hasjob then
            if not DoesBlipExist(startBlip) then
                startBlip = addBlip(Config.JobCenter, 477, 0, Strings['start_job'])
            end
        else
        ...
        if distance <= 25.0 and hasjob then
            ...
            if distance <= 1.5 then
                ...
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent('valiant_trucking:jobMenu')
                end
            end
        end
        Wait(sleep)
...
RegisterNetEvent('valiant_trucking:start')
AddEventHandler('valiant_trucking:start', function(data, jobid)
    working = true
    ...
    DeleteVehicle(truck)
    working = false
    TriggerServerEvent('valiant_trucking:jobFinished', jobid, damages)
end)

RegisterNetEvent('valiant_trucking:menu')
AddEventHandler('valiant_trucking:menu', function(data)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            TriggerServerEvent('valiant_trucking:startJob', data.current.value)
            menu.close()
        end,
    function(data, menu)
        menu.close()
